...
class BTAC {
   ...
   public:
      BTAC(unsigned _size) 
      {
         numBTACmiss=0;
         numCorrectlyTaken=0;
         numIncorrectlyTaken=0;
         numCorrectlyNotTaken=0;
         numIncorrectlyNotTaken=0;
         for(int i=0;i<128;i++)
            cache[i].targetAddr=0;
      }
      unsigned int numBTACmiss;
      unsigned int numCorrectlyTaken;
      unsigned int numIncorrectlyTaken;
      unsigned int numCorrectlyNotTaken;
      unsigned int numIncorrectlyNotTaken;

      void access(unsigned instrAddr, unsigned targetAddr, bool taken);
      void print();
};

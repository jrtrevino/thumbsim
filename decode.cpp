    ...
    if(opts.instrs) {
      ...
      if (data.instr.add.d) {
        if (data.instr.add.rd == 5) {
          ...
          cout << " r" << setbase(10) << (8+data.instr.add.rd) << ", sp, r" << setbase(10) << (8+data.instr.add.rd) << endl;
        }
        else {
        ...
        cout << " r" << data.instr.add.rd << ", sp, r" << setbase(10) << data.instr.add.rd << endl;
      }
      else {
...
int decode (const LDM_Type data) {
  ...
  if (opts.instrs) { 
    bool multiple = FALSE;
	int rn_bit = pow(2, data.instr.ldm.rn);
	cout << "ldm r" << data.instr.ldm.rn;
	if(!(data.instr.ldm.reg_list & rn_bit)){
	  cout << "!";
	}
	cout << ", {";
	if(data.instr.ldm.reg_list & 1 && rn_bit != 1){
      cout << "r0";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 2 && rn_bit != 2){
      if (multiple)
        cout << ", ";
      cout << "r1";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 4 && rn_bit != 4){
      if (multiple)
        cout << ", ";
      cout << "r2";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 8 && rn_bit != 8){
      if (multiple)
        cout << ", ";
      cout << "r3";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 16 && rn_bit != 16){
      if (multiple)
        cout << ", ";
      cout << "r4";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 32 && rn_bit != 32){
      if (multiple)
        cout << ", ";
      cout << "r5";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 64 && rn_bit != 64){
      if (multiple)
        cout << ", ";
      cout << "r6";
      multiple = TRUE;
	}
	if(data.instr.ldm.reg_list & 128 && rn_bit != 128){
      if (multiple)
        cout << ", ";
      cout << "r7";
      multiple = TRUE;

...
bool Cache::access(unsigned int address) {
  unsigned int i=1,bytesel=0,indexSize=0,index=0,tagSize=0,tag=0,numBlocks=size/blocksize;
  ...
  tagSize = 32 - bytesel - indexSize;
  if(indexSize)
    index = (address << tagSize) >> (tagSize+bytesel);
  tag = address >> (indexSize+bytesel);
...
void BTAC::access(unsigned instrAddr, unsigned targetAddr, bool taken)
{
   unsigned index = (instrAddr >> 3) & 0x7F;

   if(cache[index].targetAddr==targetAddr)
   {
      if((cache[index].taken && taken)||((!cache[index].taken) && (!taken)))
      {
         cache[index].strong = true;
         if(taken)
            numCorrectlyTaken++;
         else
            numCorrectlyNotTaken++;
      }
      else
      {
         if(taken)
            numIncorrectlyNotTaken++;
         else
            numIncorrectlyTaken++;
         if(cache[index].strong)
            cache[index].strong = false;
         else
            cache[index].taken = taken;
      }
   }
   else
   {
      numBTACmiss++;
      cache[index].targetAddr = targetAddr;
      cache[index].taken = taken;
      cache[index].strong = false;
   }
}
void BTAC::print() {
  // unsure if numBTACmiss correlats to not dynamically predicted mentioned
  // in milestone directions
  cout << "Branch Prediction:" << endl
       << "  Not Dynmaically Predicted: " << numBTACmiss << endl
       << "  Correctly Taken: " << numCorrectlyTaken << endl
       << "  Incorectly Taken: " << numIncorrectlyTaken << endl
       << "  Correctly Not Taken: " << numCorrectlyNotTaken << endl
       << "  Incorrectly Not Taken: " << numIncorrectlyNotTaken << endl; 
}
